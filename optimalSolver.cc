...
void OptimalSolver::init(int readLength, int seedNum) {
	...
	//For the first 1 seed case, we need to consider all cases
	lvPosCount = readLength + 1 - tree.getHashLength();
	matrixSize += (lvPosCount + 1) * lvPosCount / 2;

	//For the next N-2 seed cases, we need to consider all cases, while leaving
	//the last minLength out (because we don't need to)
	for (int i = 1; i < this->seedNum - 1; i++) {
	...
	//For the last seed, it's simpler all cases for dividers
	lvPosCount = readLength + 1 - this->seedNum * tree.getHashLength();
	matrixSize += lvPosCount;

	//Allocate space for the matrix
	matrix = new Cell[matrixSize];
	defaultCells = new Cell[matrixSize];
...
void OptimalSolver::feedL0() {
	...
	for (int i = 0; i < lvPosCount; i++) {
		for (int j = 0; j < lvPosCount - i; j++) {
			cin >> level[0][i][j].start;
			cin >> level[0][i][j].end;
			cin >> level[0][i][j].frequency;
			cin >> level[0][i][j].isleaf;
#ifdef DEBUG
			cout << "level[0][" << i << "][" << j << "].start=" << level[0][i][j].start << endl;
			cout.flush();
#endif
		}
	}
...
unsigned int OptimalSolver::solveDNA(string DNA) {
	...
	if (!L0Loaded) {
		...
		for (int i = 0; i < lvPosCount; i++) {
			...
			tree.query(seed);
			level[0][0][i].start = i;
			level[0][0][i].end = i + tree.getHashLength() - 1;
			level[0][0][i].frequency = tree.frequency();
			level[0][0][i].isleaf = tree.isLeaf();

#ifdef DEBUG
			cout << level[0][0][i].start << "-" << level[0][0][i].end << ":" << level[0][0][i].frequency << " ";
#endif
		}
#ifdef DEBUG
		cout << endl;
