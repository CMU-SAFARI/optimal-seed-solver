...
int main(int argc, const char* argv[]) {
	...
	ofstream output;
	output.open( (string(benchName) + string(".gems") ).c_str() );
	...
	while (seedNum > 0) {
		...
		getline(readFile, read);
		//cout << "DNA: " << read << endl;
		fsolver.init(read.length(), seedNum, 14);
		...
		if (frequency < 0) {
			assert(frequency == -1);
			frequency = fsolver.solveDNA(read);
		}
