...
int main(int argc, const char* argv[]) {
	...
	SpacedSeedSolver solver;

	if (argc != 3 && argc != 4) {
		cerr << "USAGE: ./testOptimalSolver <Ref File> <TableFile> <Mask>" << endl;
		cerr << "USAGE: ./testOptimalSolver <TableFile> <Read File>" << endl;
		exit(1);
	}

	if (argc == 4) {
		solver.generateHash(argv[1], argv[3]);
	...
	while (seedNum > 0) {	
		...
		//Solve
		frequency = solver.solveDNA(read, seedNum);
